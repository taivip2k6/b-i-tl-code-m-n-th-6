...
LEVELS = [
    ...
    },
    {
        ...
        "correct_output": "safe123",
        "hint": "Nhập từ khóa dễ nhớ cho bản đồ an toàn.",
        "story": "Một tấm bản đồ gửi qua kênh không an toàn; bạn cần kiểm tra xem liệu nội dung có bị thay đổi hay không.",
        "solution_param": "",
        "explanation": "Hash được dùng để kiểm tra toàn vẹn dữ liệu. Nếu nội dung thay đổi, giá trị băm cũng đổi, giúp phát hiện dữ liệu bị sửa.",
        "answer_label": "Từ khóa",
        "input_type": "text",
    },
    {
        ...
        "algorithm": "aes",
        "title": "Màn 4: AES",
        "ciphertext": base64.b64encode(aes_encrypt(AES_KEY, "treasure")).decode(),
        "correct_output": "treasure",
        "hint": "Nhập từ khóa dễ nhớ cho kho báu.",
    ...
    },
    {
        ...
        "algorithm": "rsa",
        "title": "Màn 5: RSA",
        "ciphertext": base64.b64encode(rsa_encrypt(rsa_public_key, "unlock")).decode(),
        "correct_output": "unlock",
        "hint": "Nhập từ khóa mở kho báu.",
...
def decode(level):
    ...
    try:
        if level_data["algorithm"] == "caesar":
            ...
            submitted_param_str = str(submitted_param).strip()
            if submitted_param_str.lower() == "unlock":
                decrypted_result = "unlock"
            else:
                if not submitted_param or not submitted_param_str.startswith("-----BEGIN RSA PRIVATE KEY-----"):
                ...
                try:
                    ...
                    decrypted_result = rsa_decrypt(submitted_param_str.encode("utf-8"), encrypted_message_bytes)
            ...
            submitted_param_str = str(submitted_param).strip()
            if submitted_param_str.lower() == "treasure":
                decrypted_result = "treasure"
            else:
                ...
                try:
                    aes_key_bytes_from_param = base64.b64decode(submitted_param_str)
            ...
            if not submitted_param:
                return jsonify({"success": False, "message": "Vui lòng nhập giá trị SHA-256!", "attempts_left": attempt.attempts_left}), 400
